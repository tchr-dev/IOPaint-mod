...
> If you don't use `uv`, run `python -m pip install -r requirements.txt` and
> then `python main.py start --model openai-compat --port 8080`.
...
In the UI, set the provider to `proxyapi` (if available). If a base URL field is
shown, use:
...
OS environment variables still take precedence over the file. Override path with
`AIE_CONFIG_FILE=path/to/env`.
