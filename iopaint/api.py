...
class Api:
    def __init__(self, app: FastAPI, config: ApiConfig):
        ...
        self.add_api_route("/api/v1/config", self.api_save_config, methods=["POST"])
    ...
    def api_server_config(self) -> ServerConfigResponse:
        ...
        model_infos = [
            ModelInfo(
                ...
                is_single_file_diffusers=it.is_single_file_diffusers,
            )
            for it in self.model_manager.available_models.values()
        ]

        # Scan for existing models to list in downloaded_model dropdown
        ...
        return ServerConfigResponse(
            ...
            enableFileManager=self.config.enable_file_manager,
            enableAutoSaving=self.config.enable_auto_saving,
            enableControlnet=self.config.enable_controlnet,
            controlnetMethod=self.config.controlnet_method,
            disableModelSwitch=self.config.disable_model_switch,
            isDesktop=self.config.is_desktop,
            samplers=SDSampler.values(),
            ...
            disable_nsfw_checker=self.config.disable_nsfw_checker,
            local_files_only=self.config.local_files_only,
            cpu_textencoder=self.config.cpu_textencoder,
            device=self.config.device,
            input=self.config.input,
            mask_dir=self.config.mask_dir,
            output_dir=self.config.output_dir,
            quality=self.config.quality,
            enable_interactive_seg=self.config.enable_interactive_seg,
            interactive_seg_device=self.config.interactive_seg_device,
            enable_remove_bg=self.config.enable_remove_bg,
            remove_bg_device=self.config.remove_bg_device,
            enable_anime_seg=self.config.enable_anime_seg,
            enable_realesrgan=self.config.enable_realesrgan,
            realesrgan_device=self.config.realesrgan_device,
            enable_gfpgan=self.config.enable_gfpgan,
            gfpgan_device=self.config.gfpgan_device,
            enable_restoreformer=self.config.enable_restoreformer,
            restoreformer_device=self.config.restoreformer_device,
        )

    def api_save_config(self, config: ApiConfig):
        from iopaint.config_loader import save_config
        try:
            save_config(config)
            self.config = config
    ...
    def launch(self):
        self.add_api_route("/api/v1/save_config", self.api_save_config, methods=["POST"])
